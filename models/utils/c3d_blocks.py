...
class APM(nn.Module):
    ...
    def __init__(self,
        ...
        self.semantic_mapping = nn.Conv3d(in_channels,
                                          out_channels,
                                          kernel_size=1,
                                          bias=False)
        if self.contrastive_att:
            self.x_mapping = nn.Conv3d(in_channels,
                                       out_channels,
                                       kernel_size=1,
                                       bias=False)
            self.n_mapping = nn.Conv3d(in_channels,
                                       out_channels,
                                       kernel_size=1,
                                       bias=False)
            self.contrastive_att_net = nn.Sequential(
                nn.Conv3d(out_channels, 1, kernel_size=1, bias=False),
                nn.Sigmoid())
    ...
    def forward(self, x):
        ...
        neighbor_time_index = torch.cat([(torch.arange(0, t) + i).unsqueeze(0)
                                         for i in range(N) if i != N // 2],
                                        dim=0).t().flatten().long()
        ...
        x_norm_expand = x_norm.unsqueeze(3).expand(
            -1, -1, -1, N - 1, -1,
            -1).permute(0, 2, 3, 4, 5,
                        1).contiguous().view(-1, h * w,
                                             c // 16)  # (b*t*2, h*w, c/16)
        neighbor_norm = x_norm_padding[:, :,
                                       neighbor_time_index, :, :].permute(
        ...
        similarity = torch.matmul(
            x_norm_expand,
            neighbor_norm) * self.temperature  # (b*t*2, h*w, h*w)
        similarity = F.softmax(similarity, dim=-1)  # (b*t*2, h*w, h*w)

        x_padding = self.padding(x)
        neighbor = x_padding[:, :, neighbor_time_index, :, :].permute(
            0, 2, 3, 4, 1).contiguous().view(-1, h * w, c)
        neighbor_new = torch.matmul(similarity, neighbor).view(
            b, t * (N - 1), h, w, c).permute(0, 4, 1, 2,
                                             3)  # (b, c, t*2, h, w)

        # contrastive attention
        if self.contrastive_att:
            x_att = self.x_mapping(
                x.unsqueeze(3).expand(-1, -1, -1, N - 1, -1,
                                      -1).contiguous().view(
                                          b, c, (N - 1) * t, h, w).detach())
        ...
        x_offset = torch.zeros([b, c, N * t, h, w],
                               dtype=x.data.dtype,
...
class C2D(nn.Module):
    ...
    def __init__(self, conv2d, **kwargs):
        ...
        self.conv3d = nn.Conv3d(conv2d.in_channels,
                                conv2d.out_channels,
...
class API3D(nn.Module):
    ...
    def __init__(self,
        ...
        self.APM = APM(conv2d.in_channels,
...
class P3DA(nn.Module):
    ...
    def __init__(self, conv2d, time_dim=3, time_stride=1, **kwargs):
        ...
        stride = (1, conv2d.stride[0], conv2d.stride[0])
        padding = (0, conv2d.padding[0], conv2d.padding[1])
        self.spatial_conv3d = nn.Conv3d(conv2d.in_channels,
                                        conv2d.out_channels,
                                        kernel_size=kernel_dim,
                                        padding=padding,
                                        stride=stride,
        ...
        self.temporal_conv3d = nn.Conv3d(conv2d.out_channels,
                                         conv2d.out_channels,
                                         kernel_size=kernel_dim,
                                         padding=padding,
                                         stride=stride,
...
class P3DB(nn.Module):
    ...
    def __init__(self, conv2d, time_dim=3, time_stride=1, **kwargs):
        ...
        stride = (time_stride, conv2d.stride[0], conv2d.stride[0])
        padding = (time_dim // 2, 0, 0)
        self.temporal_conv3d = nn.Conv3d(conv2d.in_channels,
                                         conv2d.out_channels,
                                         kernel_size=kernel_dim,
                                         padding=padding,
                                         stride=stride,
...
class APP3DA(nn.Module):
    ...
    def __init__(self,
        ...
        self.APM = APM(conv2d.out_channels,
